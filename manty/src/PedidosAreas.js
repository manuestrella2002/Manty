...
const PedidosAreas = () => {
  const { area } = useParams(); // Obtiene el area del pedido de la URL
  const [pedidosAbiertos, setPedidosAbiertos] = useState([]);
  const navigate = useNavigate();
  useEffect(() => {
    // Obtener los pedidos desde el servidor
    axios.get(`http://192.168.1.7:5000/api/pedidos/area/${area}`)  
      .then(response => {
        if (response.data.length === 0) {
          setPedidosAbiertos([]); // Asegura que se borren los pedidos si no hay datos
        } else {
          setPedidosAbiertos(response.data);
        }
      })
      .catch(error => {
        console.error('Hubo un error al obtener los pedidos del área:', area, error);
        setPedidosAbiertos([]); // También vacía la tabla si hay un error
      });
  }, [area]); // Se ejecuta cada vez que cambia el área
